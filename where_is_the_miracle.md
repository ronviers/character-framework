...
*An origin-of-life reframing. The physics and mathematics below are established and cited to their
...
Strip away free reproduction and forced information, and what remains as the genuine threshold? A
circulation that **reads its own protected bit** — that consults the very distinction it carries in
order to sustain the machinery doing the reading. The bit stops being inert and becomes
**load-bearing**: it matters not because it is inherited, but because it is *used*. This is the seam
the framework points at, and it has older names — Rosen's **closure to efficient causation** (the
system that produces the catalysts that produce it) and Pattee's **epistemic cut** (where a stored
symbol becomes running dynamics). The relocation, in one line: the miracle moves from *the first
replicator* to *the first circulation that closes on its own reading*.

Reading itself splits, and the split is the crux. The cheap half is **metric** — an environment
changes how fast a loop turns, which branch it takes, how long it dwells; this is generic,
instantaneous, and carries no heredity. A whole kingdom of such metric readers is possible: each
responsive to its world while its protected bit stays sealed from it. The hard half is **bit-level** —
the environment reaching the protected sign itself — and no instantaneous path runs to it. The only
channel is **selection across replacement**: the niche keeping one handedness over its mirror, over a
lineage. So *sorting* (a medium keeping the stable) becomes *selection* (a niche keeping one protected
sign) exactly here, and **reader and heredity arrive together, at the bit** — not reader, then memory,
then heredity in sequence.

## 4. The tape comes last, and it is forced

Then where does the genome come from — the copyable record the textbooks put first? It comes **last**,
...
carry what the keep alone could not, and the planet's cheap, abundant copying comes at last to matter.
**The living thing is not the tape; it is the depth of the keep, with the tape the crutch picked up at
its edge.**

## Where it touches ground

A relocation is only interesting if it predicts something, and this one does, in the structure of a
real cell. Draw the keep/archive line through the most reduced organism that still runs — a minimal
synthetic bacterium, its genome pared to the essential — and the line falls strangely. Most of the
irreducible genome is not the metabolism the cell runs but the **reader that runs it**: the
transcription-and-translation machinery that turns the archive back into parts takes up nearly half of
everything stored (about 41% of the archive's bits). The standing information a cell cannot do without
is, in the established vocabulary, *stored semantic information* (Kolchinsky–Wolpert); the gene
...
One honest line, because the relocation is not a solution. The account demotes reproduction and
information to cheap-or-forced, measures the structure around the threshold, and shows that physics
unaided stays **degenerate** — about one bit, below von Neumann's open-ended threshold, without an
imported tape. The one thing it does not show is that the reading loop actually closed: that on a real
dissipative medium, a frustrated cycle minted a protected bit that read its ecology and broke the
degeneracy. That step — the closure itself — is granted, not demonstrated. The engine beneath it (a
...
to be cheap — and points it instead at closure, a smaller and more tractable place to be puzzled.
Whether closure is the real threshold, or is itself cheap and the mystery moves again, is a question
the framework has at least made well-posed. That is what a good reframing does: it does not answer the
question, it relocates it to where the answer might actually be.

---

*This is the origin-of-life reading of a broader framework for driven steady states. The framework
proper — its definitions, derivations, real-substrate instances, and falsifiers — is in
[`framework/character.md`](framework/character.md); every structure named above (the protected
circulation, minting, the keep, the archive wall, the reader as closure) is borrowed and attributed in
[`framework/character_prior_art.md`](framework/character_prior_art.md). The only claim is the reading.*
