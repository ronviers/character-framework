...
**Latest (this session): `current-aids-escape` — confirmed on homochiral (3σ), NULL on autocat; outbound
interpretation prompt drafted.** Homochiral: the protected current *lowers* the FW branch-escape barrier
...
1. **`current-aids-escape` — confirmed on homochiral (3σ), NULL on autocat; outside interpretation in
   ...
   **Autocat (would-be 2nd instance): clean null** — same protocol (`ec,a+b` fixed, dial `a−b`, `𝒜:0→2.3`),
   ...
   how to disentangle the 3 confounds. **The frontier/receipts interpretive rewrite + the `character.md`
   core caveat are HELD pending that report.** After it returns: likely a clean disentangling substrate
   (vary one axis at a time) and/or **gMAM** (#2) on homochiral to *see* the instanton tilt and explain A's
   null.
