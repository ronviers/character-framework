...
lives there. **Fix-if-pursued (a+b):** the first controlled nonlinearity (`−λx³` / `−gx⁴`), where `R`
acquires non-Gaussian vertices invisible to the two-point `C` — **vindicating the anharmonic route over the
trap-model pivot** the mid-tier model pushed.
